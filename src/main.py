...
from src.algorithms.rays import uniform_hemisphere
...
def has_hit(path: RayPath) -> bool:
    return any(isinstance(n, ProbeNode) for n in path.nodes)
...
if __name__ == "__main__":
    ...
    N_H = 20000  # << 先小量验证流程，逐步升到 10^5 量级
    min_cos_to_normal = 0.08  # 掠射剔除阈值

    # 只保留“向上半球”
    dirs_h = uniform_hemisphere(N_H, up=(0, 1, 0), method="fibonacci")
    ny = np.array([0.0, 1.0, 0.0], dtype=float)
    keep_h = (dirs_h @ ny) >= min_cos_to_normal
    dirs_all = dirs_h[keep_h]

    print(f"将发射射线总数: {len(dirs_all)}（已按 |dir·ny|≥{min_cos_to_normal} 过滤掠射）")

    total_paths = total_hits_h = 0
    for d in dirs_all:
        path = trace_single_ray(
            scene=scene, origin=ORIGIN, dir=d,
            material_map=material_map, probe_geom_ids=probe_geom_ids,
            max_bounces=3
        )
        if not has_hit(path):  # 没命中任何 Probe，不算物理量
            continue

        compute_physics_for_raypath(path, CHARGE_KG_TNT)
        total_hits_h += register_probe_hits_for_path(path, probe_grid_h)
        total_paths += 1

    print("\n【覆盖统计】")
    print(f"水平面：命中过的格子数 = {len(probe_grid_h.cells)} / 总格子 {probe_grid_h.nu * probe_grid_h.nv}")
    ...
    run_meta = {
        ...
        "note": "only horizontal probe (y=10 m); directions from uniform_hemisphere; grazing filtered"
    }
    ...
    print("\n已导出：probe_hits_points_y10.csv / probe_hits_cells_y10.csv")
